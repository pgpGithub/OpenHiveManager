...
class TranshumanceFieldSubscriber implements EventSubscriberInterface
{
    ...
    private function addEmplacementForm($form, $rucherto = null)
    {
        $formOptions = array(
            ...
            'required' => false,
            'query_builder' => function (EntityRepository $repository) use ($rucherto) {
                $qb = $repository->queryfindByRucherId($rucherto);
                return $qb;
            }
        );
    ...
    public function preSubmit(FormEvent $event)
    {
        ...
        $rucherto = array_key_exists('rucherto', $data) ? $data['rucherto'] : null;
        $this->addEmplacementForm($form, $rucherto);
    }
}